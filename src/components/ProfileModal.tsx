...
export default function ProfileModal({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    let mounted = true;
    const skipped = localStorage.getItem("profile-prompt-dismissed") === "true";
    if (skipped) {
      onComplete?.();
      return;
    }
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((_evt, session) => {
      ...
      const meta = (user?.user_metadata || {}) as any;
      const hasProfile = Boolean(meta?.username) && (meta?.age !== undefined && meta?.age !== null);
      const stillSkipped = localStorage.getItem("profile-prompt-dismissed") === "true";
      if (user && !hasProfile && !stillSkipped) {
        setOpen(true);
      }
    });
